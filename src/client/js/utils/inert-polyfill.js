...
export default () => {
  ...
  window.addEventListener('load', () => {
    ...
    const css = '/*[inert]*/*[inert]{-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;pointer-events:none}'
    ...
    */
    function sharedFocusHandler (target) {
      const inertElement = madeInertBy(target)
      ...
      if (document.hasFocus() && lastTabDirection !== 0) {
        ...
        const previous = getFocused()

        dispatchTabEvent(lastTabDirection < 0)
        ...
        const filter = /** @type {NodeFilter} */ {
          ...
          */
          'acceptNode' (node) {
            ...
            const contained = inertElement.contains(node)
        ...
        const nextFunc = Math.sign(lastTabDirection) === -1 ? walker.previousNode : walker.nextNode
        const next = nextFunc.bind(walker)
    ...
    document.body.addEventListener('focus', (ev) => {
      const target = targetForEvent(ev)
